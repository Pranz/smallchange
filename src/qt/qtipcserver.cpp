...
static void ipcThread2(void* pArg)
{
    ...
    message_queue::remove(TOAKRONAURI_QUEUE_NAME);
...
void ipcInit()
{
    ...
    try {
        mq = new message_queue(open_or_create, TOAKRONAURI_QUEUE_NAME, 2, MAX_URI_LENGTH);

        // Make sure we don't lose any toakrona: URIs
        for (int i = 0; i < 2; i++)
        {
